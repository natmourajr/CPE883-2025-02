...
openwebtextconstants = DatasetConstants(
    chars_per_sample=2163,  # Estimate, adjust if you have a better value
    chars_per_token=4
)
openwebtextconstants.splits['train'] = DataSplitConstants(
    hf_split='train',
    folder_split='train',
    raw_samples=8000000,  # Approximate, adjust as needed
    truncated_samples=None
)

openwebtextconstants.splits['val'] = DataSplitConstants(
    hf_split='validation',
    folder_split='val',
    raw_samples=0,  # OpenWebText may not have a validation split
    truncated_samples=None
)
...
CONSTS['Skylion007/openwebtext'] = openwebtextconstants

CONSTS['allenai/c4'] = c4constants
...
def main(args: Namespace) -> None:
    ...
    for split_name in args.splits:
        ...
        # Get samples
        openwebtext_names = ['openwebtext', 'Skylion007/openwebtext']
        dataset = build_hf_dataset(dataset_name=args.dataset,
                                   data_subset=args.data_subset if args.dataset not in openwebtext_names else None,
                                   split=hf_split,
                                   mode=mode,
