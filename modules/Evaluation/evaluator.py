...
"""
        ...
        for set_name, data_loader in sets_to_evaluate.items():
            ...
            # Análise Geral
            y_true_geral = results_df['true_label'].values
            y_probs_geral = results_df['probability'].values
            
            if len(np.unique(y_true_geral)) < 2:
                print(f"  AVISO: O conjunto de '{set_name}' no Fold {fold_num} contém apenas uma classe. Pulando métricas.")
                auc, sens, spec = np.nan, np.nan, np.nan
            else:
                plot_roc_curve(y_true_geral, y_probs_geral, fold_num, set_name, fold_dir)
                auc = roc_auc_score(y_true_geral, y_probs_geral)
                threshold = config['training'].get('decision_threshold', 0.5)
                y_pred_class = (np.array(y_probs_geral) >= threshold).astype(int)
                tn, fp, fn, tp = confusion_matrix(y_true_geral, y_pred_class, labels=[0, 1]).ravel()
                sens = tp / (tp + fn) if (tp + fn) > 0 else 0.0
                spec = tn / (tn + fp) if (tn + fp) > 0 else 0.0

            set_key = 'validation' if set_name == 'Validação' else 'holdout'
            fold_metrics[f'auc_{set_key}'] = auc
            fold_metrics[f'sensitivity_{set_key}'] = sens
            fold_metrics[f'specificity_{set_key}'] = spec
            print(f"\n--- Análise de Performance Geral ({set_name}) - Fold {fold_num} ---")
            print(f"  -> AUC: {auc:.4f}, Sensibilidade: {sens:.4f}, Especificidade: {spec:.4f}")
            ...
            results_df['age_group'] = pd.cut(results_df['age'], bins=age_bins, labels=age_labels, right=True).astype(str)
            subgroups = {
                "Gênero: Masculino": results_df[results_df['gender'] == 'Male'],
                "Gênero: Feminino": results_df[results_df['gender'] == 'Female'],
                "Idade: 0-40": results_df[results_df['age_group'] == '0-40'],
                "Idade: 41-60": results_df[results_df['age_group'] == '41-60'],
                "Idade: 61+": results_df[results_df['age_group'] == '61+'],
            }
            print(f"\n--- Análise de Performance por Subgrupo ({set_name}) - Fold {fold_num} ---")
            ...
            for group_name, df_group in subgroups.items():
                ...
                # Gera uma chave limpa para o dicionário (ex: 'auc_validation_genero_masculino')
                set_key = 'validation' if set_name == 'Validação' else 'holdout'
                group_key = group_name.lower().replace(': ', '_').replace('-', '_')

                if len(df_group) > 0 and len(np.unique(y_true_group)) > 1:
                    auc_group = roc_auc_score(y_true_group, y_probs_group)
                    # Salva a métrica do subgrupo no dicionário do fold
                    fold_metrics[f'auc_{set_key}_{group_key}'] = auc_group
                    
                    # Para o grupo "Geral", também salva as outras métricas
                    if group_name == "Geral":
                        plot_roc_curve(y_true_group, y_probs_group, fold_num, set_name, fold_dir)
                        threshold = config['training'].get('decision_threshold', 0.5)
                        y_pred_class = (np.array(y_probs_group) >= threshold).astype(int)
                    ...
                    print(f"  -> {group_name:<20} | AUC: {auc_group:.4f} | N: {len(df_group)}")
                else:
                    # Se não puder calcular, salva NaN
                    fold_metrics[f'auc_{set_key}_{group_key}'] = np.nan
                    print(f"  -> {group_name:<20} | AUC: N/A (dados insuficientes: {len(df_group)})")
    ...
    final_summary = {}
    for key in metrics_to_summarize:
        # Coleta os valores de cada fold para a métrica atual
        values = [res.get(key, np.nan) for res in fold_results]
        
        # Calcula média e desvio padrão, ignorando 'NaN'
        mean_val = np.nanmean(values)
        std_val = np.nanstd(values)
        ...
        # Salva no dicionário final, convertendo para float padrão
        final_summary[f'mean_{key}'] = float(mean_val)
        final_summary[f'std_{key}'] = float(std_val)
