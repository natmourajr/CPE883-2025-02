...
from rock_seg_model.deeponet_rock_seg.model_deeponet import DeepONetConv    # <- trocar para o modelo desejado
from rock_seg_model.deeponet_rock_seg.deeponet_config import DEEPONET_CONFIG  # <- trocar para o config do modelo
...
dataset = RockSegmentationDatasetMulti(
    ...
    output_shape=DEEPONET_CONFIG["output_shape"]
)
...
model = DeepONetConv(DEEPONET_CONFIG).to(device)
...
results_dir = os.path.join("Results", "deeponet")
...
for epoch in range(1, num_epochs + 1):
    ...
    with torch.no_grad():
        for imgs, masks, filename in val_loader:
            ...
            dices.append(dice_score(preds, masks, DEEPONET_CONFIG["num_classes"]))
            ious.append(iou_score(preds, masks, DEEPONET_CONFIG["num_classes"]))
    ...
    with torch.no_grad():
        for imgs, masks, filename in test_loader:
            ...
            preds = torch.argmax(outputs, dim=1)
            test_dices.append(dice_score(preds, masks, DEEPONET_CONFIG["num_classes"]))
            test_ious.append(iou_score(preds, masks, DEEPONET_CONFIG["num_classes"]))
...
plt.xlabel("Época"); plt.ylabel("Loss (Escala Log)")
plt.yscale('log')
