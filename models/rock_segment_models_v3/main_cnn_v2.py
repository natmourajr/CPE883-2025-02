...
from rock_seg_model.cnn_rock_seg.model_cnn import CNNSegmentationModel
...
dataset = RockSegmentationDatasetMulti(
    ...
    output_shape=CNN_CONFIG["output_shape"]
)
...
model = CNNSegmentationModel(CNN_CONFIG).to(device)
...
for epoch in range(1, num_epochs + 1):
    ...
    with torch.no_grad():
        for imgs, masks, filename in val_loader:
            ...
            dices.append(dice_score(preds, masks, CNN_CONFIG["num_classes"]))
            ious.append(iou_score(preds, masks, CNN_CONFIG["num_classes"]))
    ...
    with torch.no_grad():
        for imgs, masks, filename in test_loader:
            ...
            preds = torch.argmax(outputs, dim=1)
            test_dices.append(dice_score(preds, masks, CNN_CONFIG["num_classes"]))
            test_ious.append(iou_score(preds, masks, CNN_CONFIG["num_classes"]))
