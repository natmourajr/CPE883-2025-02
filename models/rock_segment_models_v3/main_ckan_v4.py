...
from rock_seg_model.ckan_rock_seg.model_ckan_v1 import CKANSegmentationModel   # <- trocar para o modelo desejado
from rock_seg_model.ckan_rock_seg.ckan_config import CKAN_CONFIG  # <- trocar para o config do modelo
...
dataset = RockSegmentationDatasetMulti(
    ...
    output_shape=CKAN_CONFIG["output_shape"]
)
...
model = CKANSegmentationModel(CKAN_CONFIG).to(device)
...
results_dir = os.path.join("Results", "CKAN")
...
for epoch in range(1, num_epochs + 1):
    ...
    with torch.no_grad():
        for imgs, masks, filename in val_loader:
            ...
            dices.append(dice_score(preds, masks, CKAN_CONFIG["num_classes"]))
            ious.append(iou_score(preds, masks, CKAN_CONFIG["num_classes"]))
    ...
    with torch.no_grad():
        for imgs, masks, filename in test_loader:
            ...
            preds = torch.argmax(outputs, dim=1)
            test_dices.append(dice_score(preds, masks, CKAN_CONFIG["num_classes"]))
            test_ious.append(iou_score(preds, masks, CKAN_CONFIG["num_classes"]))
