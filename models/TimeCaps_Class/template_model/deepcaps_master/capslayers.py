...
from deepcaps_master.batchdot import own_batch_dot
...
def normalize_tuple(value, n, name):
    """Transforma um valor em uma tupla de tamanho n. Substituto de conv_utils.normalize_tuple"""
    if isinstance(value, int):
        ...
        raise ValueError(f'The `{name}` argument must be an int or a tuple of {n} integers. Received: {value}')

def normalize_padding(padding):
    """Substituto de conv_utils.normalize_padding"""
    if padding is None:
        return 'valid'
    padding = padding.lower()
    if padding not in {'valid', 'same', 'full'}:
        raise ValueError('Invalid padding: {}. Expected "valid", "same", or "full".'.format(padding))
    return padding
...
class Conv2DCaps(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.h_j, self.w_j = [conv_output_length(input_shape[i + 1],
                                                            self.kernel_size[i],
                                                            padding=self.padding,
                                                            stride=self.strides[i],
                                                            dilation=self.dilation_rate[i]) for i in (0, 1)]

        self.ah_j, self.aw_j = [conv_output_length(input_shape[i + 1],
                                                              self.kernel_size[i],
                                                              padding=self.padding,
                                                              stride=1,
                                                              dilation=self.dilation_rate[i]) for i in (0, 1)]
