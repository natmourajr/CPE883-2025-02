...
from deepcaps_master.capslayers import FlattenCaps
...
from tensorflow.keras.layers import UpSampling2D, Conv2D, MaxPooling2D, Conv3D
...
from CapsNet_Keras_master.capsulelayers import CapsuleLayer, PrimaryCap, Length, Mask
from sklearn.model_selection import train_test_split
from deepcaps_master.capslayers import *
# K.set_image_data_format('channels_last')


# Substituindo conv_utils do Keras
...
def deconv_length(input_length, filter_size, padding, output_padding, stride):
    if input_length is None:
        return None
    if padding == 'valid':
...
#    layers.Concatenate(axis=-2)([la, lb])
    ...
    decoder.add(layers.Conv2DTranspose(32, 1, 3,subsample=(1, 1),border_mode='same'))
    decoder.add(layers.normalization.BatchNormalization())
    decoder.add(layers.Conv2DTranspose(16, 1, 3,subsample=(1, 2),border_mode='same'))
    decoder.add(layers.normalization.BatchNormalization())
    decoder.add(layers.Conv2DTranspose(8, 1, 5,subsample=(1, 2),border_mode='same'))
    decoder.add(layers.normalization.BatchNormalization())
    decoder.add(layers.Conv2DTranspose(4, 1, 7,subsample=(1, 2),border_mode='same'))
    decoder.add(layers.normalization.BatchNormalization())
    decoder.add(layers.Conv2DTranspose(1, 1, 7,subsample=(1, 1),border_mode='same'))
    decoder.add(layers.normalization.BatchNormalization())
