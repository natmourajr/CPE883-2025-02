...
"""
CapsuleNet (CIFAR-10) com melhorias para integração ao Optuna:
- Seleção de otimizador por args.optimizer_name (Adam, RMSprop, SGD)
- run_kfold(...) retorna a acurácia média para o Optuna maximizar
"""
...
def show_reconstruction(model, test_loader, n_images, args):
    ...
    for x, _ in test_loader:
        ...
        pil_img = pil_img.resize(new_size, Image.NEAREST)  # pode trocar para BILINEAR ou LANCZOS

        pil_img.save(args.save_dir + "/real_and_recon.png")
        print()
...
def run_kfold(dataset, model, args, k=5, shuffle=True):
    ...
    for fold, (train_idx, val_idx) in enumerate(
        ...
        fold_accuracies.append(float(best_val_acc))
        fold_loss.append(float(best_val_loss))
        print(
...
def train(model, train_loader, val_loader, args, fold_n, classes):
    ...
    :return: best validation acc and loss
    """
    print("Begin Training" + "-" * 70)

    scaler = GradScaler(enabled=getattr(args, "use_amp", False))
    ...
    # >>> Seleção de otimizador conforme args.optimizer_name <<<
    opt_name = getattr(args, "optimizer_name", "Adam")
    if opt_name == "Adam":
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for i, (x, y) in enumerate(
            tqdm(train_loader, desc=f"Epoch {epoch + 1}/{args.epochs}", disable=True)
        ):  # batch training
            x, y = x.to(DEVICE), y.to(DEVICE)
            y = torch.zeros(y.size(0), 10, device=DEVICE).scatter_(
                1, y.view(-1, 1), 1.0
            ...
            if use_amp:
                with autocast(device_type="cuda"):
                    # forward e cálculo da loss exatamente como já é hoje
                    # ex.: lengths, recon = model(x) ... total_loss = cls_loss + recon_loss*...
                    loss = caps_loss(y, y_pred, x, x_recon, args.lam_recon)  # compute loss
                    # loss.backward()  # backward, compute all gradients of loss w.r.t all Variables
        ...
        with torch.no_grad():
            if use_amp:
                with autocast(device_type="cuda"):
                    # forward de validação como já existe
                    pass
            else:
                # forward de validação normal
                pass
