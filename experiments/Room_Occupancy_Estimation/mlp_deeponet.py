...
# Importar modelos do arquivo mlp_deeponet.py na pasta Room_Occupancy_Estimation
from models.Room_Occupancy_Estimation.mlp_deeponet import MLP_Model, DeepONet, DeepONetWrapper
...
print(f"A dimensão de entrada para la MLP será: {mlp_input_dim}")
...
for fold, (train_index, val_index) in enumerate(kf.split(dataset)):
    ...
    # Armazena la perda de validação final y las curvas completas para la análisis final
    ...
    # Imprime los resultados de este fold
...
# --- Matriz de Confusão Média con Desvio Padrão ---
...
# --- Matriz de Confusão Normalizada con Média y Desvio Padrão ---
...
plt.show()
print(f"Curva de perda do melhor fold salva em: {best_curve_path}")
...
"""## Treinamento DeepONet"""

# Dados temporais

print("=== PREPARAÇÃO DOS DADOS TEMPORAIS ===")

# Primeiro, criar as sequências temporais
X_seq, y_seq = create_sequences(X_data, y_data, window_size=25)
X_tensor = torch.tensor(X_seq, dtype=torch.float32)
y_tensor = torch.tensor(y_seq, dtype=torch.long)

# Criar dataset temporal
dataset_temporal = TensorDataset(X_tensor, y_tensor)

print(f"Shape do X_tensor temporal: {X_tensor.shape}")
print(f"Shape do y_tensor: {y_tensor.shape}")

# Agora sim podemos verificar a estrutura
print("=== VERIFICAÇÃO DA ESTRUTURA DOS DADOS ===")
sample_batch, sample_labels = next(iter(DataLoader(dataset_temporal, batch_size=32, shuffle=False)))
print(f"Shape do batch de treino: {sample_batch.shape}")
print(f"Shape das labels: {sample_labels.shape}")

# Pegue as dimensões diretamente do seu batch temporal
batch_size, window_size, features_per_timestep = sample_batch.shape

# A dimensão de entrada para a BRANCH NET será o total de features na janela, achatadas
branch_input_dim = window_size * features_per_timestep

# A dimensão de entrada para a TRUNK NET é o número de features de um único timestep
trunk_input_dim = features_per_timestep

n_classes = len(np.unique(y_data))

print(f"Dimensão de entrada para Branch Net (achatado): {branch_input_dim}")
print(f"Dimensão de entrada para Trunk Net: {trunk_input_dim}")
print(f"Número de classes: {n_classes}")
print(f"Validação: {window_size} * {features_per_timestep} = {branch_input_dim}")

# Importar métricas adicionais para F1 Score
from sklearn.metrics import f1_score, classification_report, confusion_matrix

# Listas para armazenar resultados da DeepONet
deeponet_val_reports = []
deeponet_val_cms = []
deeponet_val_losses_final = []
deeponet_train_losses_full = []
deeponet_val_losses_full = []
deeponet_val_accuracies = []
deeponet_val_f1_scores = []

# KFold para validação cruzada
kf = KFold(n_splits=5, shuffle=True, random_state=42)
n_splits = 5  # Definindo n_splits

# --- Loop de validação cruzada para DeepONet ---
for fold, (train_index, val_index) in enumerate(kf.split(dataset_temporal)):
    print(f"\n--- Iniciando DeepONet - Fold {fold + 1}/{n_splits} ---")

    # Cria subconjuntos do dataset temporal
    train_dataset = Subset(dataset_temporal, train_index)
    val_dataset = Subset(dataset_temporal, val_index)

    # DataLoaders com dados temporais
    train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=32, shuffle=False)

    # Pesos de classe
    y_train_fold = y_tensor[train_index].numpy()
    class_counts_fold = np.array([np.sum(y_train_fold == i) for i in range(n_classes)])
    class_weights_fold = 1.0 / class_counts_fold
    class_weights_fold = class_weights_fold / class_weights_fold.sum()
    class_weights_fold = torch.tensor(class_weights_fold, dtype=torch.float32)
    ...
    # Parâmetros específicos para o wrapper
    wrapper_kwargs = {
        'activation_fn': "ReLU",
        'dropout': 0.3,
        'use_batchnorm': True,
        ...
        'weight_decay': 1e-5,
        'num_epochs': 50,
        'class_weights': class_weights_fold.to(device),
        'window_size': window_size,
        'features_size': features_per_timestep
    }

    model = DeepONetWrapper(
        branch_input_dim=branch_input_dim,
        trunk_input_dim=trunk_input_dim,
        branch_hidden_layers=[128, 64],
    ...
    for epoch in range(model.num_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            print(f"Fold {fold+1}, Epoch {epoch+1}/{model.num_epochs}, Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}")

    # Armazena as perdas
    deeponet_train_losses_full.append(fold_train_losses)
    deeponet_val_losses_full.append(fold_val_losses)
    deeponet_val_losses_final.append(fold_val_losses[-1])
    ...
    with torch.no_grad():
        for xb, yb in val_loader:
            ...
            branch_input = xb.view(xb.size(0), -1)
            trunk_input = xb[:, -1, :]
    ...
    # Calcula métricas
    accuracy_fold = accuracy_score(y_true_fold, y_pred_fold)
    f1_fold = f1_score(y_true_fold, y_pred_fold, average='weighted')  # F1 Score ponderado
    ...
    deeponet_val_reports.append(report_fold)
    deeponet_val_cms.append(cm_fold)
    deeponet_val_accuracies.append(accuracy_fold)
    deeponet_val_f1_scores.append(f1_fold)

    print(f"\nDeepONet - Fold {fold+1} Results:")
...
# --- Resultados Finais da Validação Cruzada ---
print("\n" + "="*60)
print("RESULTADOS FINAIS DA VALIDAÇÃO CRUZADA - DeepONet")
print("="*60)

print(f"\nAcurácias por Fold: {[f'{acc:.4f}' for acc in deeponet_val_accuracies]}")
print(f"F1 Scores por Fold: {[f'{f1:.4f}' for f1 in deeponet_val_f1_scores]}")
print(f"\nAcurácia Média: {np.mean(deeponet_val_accuracies):.4f} ± {np.std(deeponet_val_accuracies):.4f}")
print(f"F1 Score Médio: {np.mean(deeponet_val_f1_scores):.4f} ± {np.std(deeponet_val_f1_scores):.4f}")
print(f"Perda de Validação Média: {np.mean(deeponet_val_losses_final):.4f}")

# --- Matriz de Confusão Consolidada ---
print("\n--- Matriz de Confusão Consolidada (Todos os Folds) ---")
all_y_true = []
all_y_pred = []

# Para obter todas as previsões de todos os folds
for fold, (train_index, val_index) in enumerate(kf.split(dataset_temporal)):
    val_dataset = Subset(dataset_temporal, val_index)
    val_loader = DataLoader(val_dataset, batch_size=32, shuffle=False)
...
sns.heatmap(final_cm_normalized, annot=True, fmt='.2%', cmap='Blues',
            xticklabels=range(n_classes), yticklabels=range(n_classes))
plt.title('Matriz de Confusão Consolidada - DeepONet (Todos os Folds)')
plt.xlabel('Previsão')
plt.ylabel('Verdadeiro')
plt.show()

# Relatório de classificação final
print("\n--- Relatório de Classificação Final (Todos os Folds) ---")
