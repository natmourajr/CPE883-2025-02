...
from models.vit import FineTunedVisionTransformer
from models.capsnet import CapsuleNet
...
def model_select(config, branch_net = None):
    ...
    if config.model_name == "MLP":
        ...
        return lambda: FineTunedVisionTransformer(num_classes=4)

    elif config.model_name == "CapsNet":
        return lambda input_size: CapsuleNet(input_shape=input_size,
                                          n_classes=4,
...
def run_experiment(config, lofar_data, results_path, device):
    ...
    accuracies, embeddings, all_targets = [], [], []
    ...
    for i, (X_train, y_train, X_test, y_test, _, _) in enumerate(lorocv_no_window.split(lofar_data)):
        ...
        if config.model_name in ["CNN", "CKAN", "CapsNet"]:
        ...
        is2d = window_size is not None or config.model_name in ["CNN", "CKAN", "CapsNet", "ViT"]
        
        transform = T.Resize((224, 224), antialias=True) if config.model_name == "ViT" else None
        
        train_dataset_fold = CustomDataloader(X_train, y_train, is2d=is2d, device=device, transform=transform)
        test_dataset_fold = CustomDataloader(X_test, y_test, is2d=is2d, device=device, transform=transform)
        ...
        input_size = X_train.shape[1:] if config.model_name not in ["MLP", "DeepONet-MLP"] else X_train.shape[1]
        
        if config.model_name == "ViT":
            model_fold = model_builder().to(device)
        else:
...
def make_hp_name(config):
    ...
    if config.model_name == "DeepONet-MLP":
        ...
        return f"lr_{learning_rate}"
        
    elif config.model_name == "CapsNet":
        return f"routings_{config.routings}_lr_{learning_rate}"
...
if __name__ == '__main__':
    ...
    parser.add-argument('--debug', action='store_true', help='Enable debug mode')
