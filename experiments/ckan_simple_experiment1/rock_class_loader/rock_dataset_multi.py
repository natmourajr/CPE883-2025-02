...
class RockSegmentationDatasetMulti(Dataset):
    ...
    #Retorna
    #A imagem normalizada [1, H, W]
    #Um rótulo único (classe dominante) extraído da máscara (0 = fundo, 1 = rocha, 2 = poro)
    #O nome da amostra, como antes
    ...
    def __getitem__(self, idx):
        ...
        # Determina a classe dominante na máscara
        unique, counts = np.unique(mask, return_counts=True)
        dominante = int(unique[np.argmax(counts)])

        if self.transform:
            image, dominante = self.transform(image, dominante)

        return (
            torch.tensor(image, dtype=torch.float32),        # imagem [1, H, W]
            torch.tensor(dominante, dtype=torch.long),       # rótulo único (classe dominante)
            amostra_nome
        )
