...
class TSDiffusion(nn.Module):
    ...
    def sample(
        self,
        batch_size: int,
        Tmax_scale: float = 1.1,        # fator δ do paper ‑ > T̂_max = μ_Tmax·(1+δ)
        lam_pad: float = 1.2,           # fator de segurança p/ λ_max adaptativo
        sampling_steps: int = None,
        device: torch.device = None
    ):
        """
        Gera séries irregulares (t_i, x_i) via reverse‑diffusion + thinning.
        Saída: lista de length=batch_size; cada item é (times, values)
        """
        device = device or next(self.parameters()).device

        # ---------- 1. Reverse‑diffusion ----------
        steps = min(sampling_steps or self.num_steps, self.num_steps)
        seq_len = 1                         # 1 estado inicial suficiente
        ...
        # z: (B,1,latent_dim)  → h0
        h0 = z.squeeze(1)                   # (B, latent_dim)

        # ---------- 2. Thinning ----------
        samples = []
        for b in range(batch_size):
            t = 0.0
            h = h0[b]                       # estado inicial deste exemplo
            Tmax_pred = self.tmax_head(h).item() * Tmax_scale
            times, values = [], []

            # estimativa inicial de λ_max
            lam_curr = F.softplus(self.lambda_head(h)).item()
            lam_max  = lam_curr * lam_pad + 1e-4

            while t < Tmax_pred:
                # 2.1 inter‑chegada do Poisson homogêneo
                w = torch.empty(1, device=device).exponential_(lam_max).item()
                t = t + w
                if t >= Tmax_pred:
                    break

                # 2.2 Evolui ODE até t  (Euler 1‑step usando w)
                #     Se você tiver decoder_odefunc, troque pelo odeint
                k1 = self.odefunc(None, h)     # f(h)
                h  = h + torch.tensor(w, device=device) * k1

                lam_t = F.softplus(self.lambda_head(h)).item()
                mu_x  = self.mean_head(h)                     # tensor (latent_dim)

                # 2.3 Aceita/rejeita
                if torch.rand(1).item() < lam_t / lam_max:
                    x_i = mu_x + torch.randn_like(mu_x)
                    values.append(x_i)
                    times.append(t)

                # 2.4 Ajuste adaptativo de λ_max
                if lam_t > lam_max:
                    lam_max = lam_t * lam_pad

            # decodifica marcas p/ espaço original se houver decoder
            if hasattr(self, 'decoder'):
                values = torch.stack(values) if values else torch.empty(0, self.latent_dim, device=device)
                values = self.decoder(values)
            else:
                values = torch.stack(values) if values else torch.empty(0, self.latent_dim, device=device)

            samples.append((torch.tensor(times, device=device), values))
    ...
    def train_model(
        ...
        for epoch in range(1, epochs + 1):
            ...
            for batch in train_loader:
                ...
                # ---------- cabeças ----------
                lam_t = F.softplus(self.lambda_head(state))            # (B,T,1)  λ(t) ≥ 0
                mu_x  = self.mean_head(state)                          # (B,T,latent_dim)

                # ---------- probabilidade dos valores ----------
                log_px = -0.5 * ((x_lat - mu_x) ** 2).sum(-1, keepdim=True)        # (B,T,1)

                # ---------- INTEGRAL ∫ λ(t) dt ----------
                lam2 = lam_t.squeeze(-1)      
                lam2.clamp(min=1e-6, max=50)                          # (B,T)
                ...
                # ---------- Loss L1 completa ----------
                log_event  = (log_px.squeeze(-1) + torch.log(lam2 + 1e-8)).sum(-1)  # (B,)
                LL         = log_event - integral_total                            # (B,)
                L1         = -LL.mean()
