...
def parse_args():
    ...
    parser.add_argument(
        "--grid_size", type=int, default=1, help="Number of grid points for KAN"
    )
    parser.add_argument("--k", type=int, default=5, help="Order of B-splines for KAN")
...
def create_dataset_from_split(all_features, all_labels, train_idx, test_idx, device):
    # Split data
    ...
    train_label = torch.from_numpy(train_labels).float().unsqueeze(1).to(device)
    val_input = torch.from_numpy(val_features).float().to(device)
    val_label = torch.from_numpy(val_labels).float().unsqueeze(1).to(device)
...
def main():
    ...
    for fold, (train_idx, test_idx) in enumerate(kfold.split(all_features)):
        print(f"\n=== Fold {fold + 1} ===")
        dataset, shape = create_dataset_from_split(
            all_features, all_labels, train_idx, test_idx, device
        )
