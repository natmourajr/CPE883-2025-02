...
# TKAN Puro - Real
# def TKAN(serie_size=-1, window_size=6, predict_steps=1, batch_size=32, epochs=20,
#         input_size=1, hidden_size=64, num_layers=2, output_size=1, Q=4, test_ratio=0.2,
#         folds=4):
#     """
...
#     model = TKANModel(input_size, hidden_size, output_size, Q)
...
#         model = TKANModel(input_size, hidden_size, output_size)
...
#     final_model = TKANModel(input_size, hidden_size, output_size)
...
# TKAN + LSTM
def TKAN(serie_size=-1, window_size=6, predict_steps=1, batch_size=32, epochs=20,
        input_size=1, tkan_hidden_size=32, lstm_hidden_size=64, num_layers=3, output_size=1, Q=4, test_ratio=0.2,
        folds=4):
    """
    ...
    model = TKANModel(input_size, tkan_hidden_size, lstm_hidden_size, output_size, Q, num_layers)
    ...
    for fold, (train_idx, val_idx) in enumerate(tscv.split(train_val_ds)):
        ...
        # Inicializa modelo novo
        model = TKANModel(input_size, tkan_hidden_size, lstm_hidden_size, output_size, num_layers)
    ...
    start_time = time.time()
    full_train_loader = DataLoader(train_val_ds, batch_size=batch_size, shuffle=False)
    final_model =TKANModel(input_size, tkan_hidden_size, lstm_hidden_size, output_size, num_layers)
    train_model(final_model, full_train_loader, test_loader=None, epochs=epochs)  # Sem validação agora
    end_time = time.time()
